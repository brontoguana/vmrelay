...
- VM inventory and VM detail state refresh quietly in the background about every 10 seconds while open. Powered-off VMs display as `off`, requested starts display as `launch...`, requested graceful shutdowns display as `shutdown...`, and rows are colored by state with off rows muted.
