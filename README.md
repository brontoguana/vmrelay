...
- VM import from the host VMs or Config tab has a read-only remote source picker opened with Enter on the Source field. It can select `.vbox`, `.vdi`, `.vmdk`, and `.vmx` files. VMRelay reads VM name, RAM, CPU count, firmware mode, and attached disks from `.vbox` and `.vmx` files; bare `.vdi`/`.vmdk` imports use conservative defaults unless the user overrides the name. Parsed source names are normalized into libvirt-safe VM names by converting spaces and invalid separators to dashes. Source networking is ignored, disks are copied and converted to qcow2 under the VMRelay/libvirt storage pool, and the new system-libvirt VM gets VMRelay NAT networking, VNC graphics, USB tablet input, restart-on-reboot lifecycle, and ownership metadata. The source files are not modified.
