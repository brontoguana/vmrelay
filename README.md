...
13. In Mappings, press `n` to add a VM-accessible service mapping and `e` to start or stop it.
...
- VM service mappings are saved per workstation/user and run as SSH reverse forwards. VMs connect to the selected host's stable libvirt bridge address, normally `192.168.122.1:<vm-port>`, and VMRelay forwards that back to `127.0.0.1:<local-service-port>` on the local workstation. The bridge address is shared by every VM on the host's default NAT network, so it does not change per VM or per guest reboot.
