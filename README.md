...
- VirtualBox import from the host VMs or Config tab reads a remote `.vbox` file for VM name, RAM, CPU count, EFI/BIOS mode, and attached hard disks; ignores VirtualBox networking; converts copied disks to qcow2 under the VMRelay/libvirt storage pool; defines a system-libvirt VM with VMRelay NAT networking, VNC graphics, USB tablet input, and restart-on-reboot lifecycle; and records ownership for the remote SSH user. The source `.vbox` and VirtualBox disk files are not modified.
